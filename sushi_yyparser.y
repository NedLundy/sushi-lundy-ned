...
cmdline:
    %empty                                /* an empty line is valid */
  | redir_exe bg_mode                     { my_shell.spawn($1, $2); }
  | in_exe pipe bg_mode                   { $1->set_pipe($2); my_shell.spawn($1, $3); }
  | YY_SUSHI_SET arg arg                  { my_shell.putenv($2, $3); }
  | YY_SUSHI_PWD                          /* Not implemented */
  | YY_SUSHI_CD arg                       /* Not implemented */
  | YY_SUSHI_HISTORY                      { my_shell.show_history(); }
  | YY_SUSHI_BANG                         { my_shell.re_parse($1); my_shell.re_execute(); }
...
arg:
    YY_SUSHI_TOK {
        if (!(*$1).empty() && (*$1)[0] == '$') {
            std::string var_name = $1->substr(1);  // Strip leading $
            delete $1;
            $$ = Sushi::getenv(var_name.c_str());
        } else {
            $$ = $1;
        }
    }
