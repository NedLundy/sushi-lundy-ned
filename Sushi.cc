...
static pid_t run_child (Program *prog, int fd_in, int fd_out, int fd_close)
{
  ...
  // Redirect the I/O
    ...
    if (prog->redir.redir_in) {
      int fd = open(prog->redir.redir_in->c_str(), O_RDONLY);
      if (fd < 0 || dup2(fd, STDIN_FILENO) == -1) {
        std::perror("input redirection");
        _exit(EXIT_FAILURE);
      }
      close(fd);
    ...
    if (prog->redir.redir_out1) {
      int fd = open(prog->redir.redir_out1->c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
      if (fd < 0 || dup2(fd, STDOUT_FILENO) == -1) {
        std::perror("output redirection >");
        _exit(EXIT_FAILURE);
      }
      close(fd);
    }
    ...
    else if (prog->redir.redir_out2) {
      int fd = open(prog->redir.redir_out2->c_str(), O_WRONLY | O_CREAT | O_APPEND, 0644);
      if (fd < 0 || dup2(fd, STDOUT_FILENO) == -1) {
        std::perror("output redirection >>");
        _exit(EXIT_FAILURE);
      }
